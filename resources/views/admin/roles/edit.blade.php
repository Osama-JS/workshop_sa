...
@section('page_title', __('admin.edit_role') . ': ' . $role->display_name)
@section('page_subtitle', 'تعديل مصفوفة الصلاحيات وبيانات الدور')
    ...
    <div class="bg-white rounded-2xl p-6 border border-slate-200/80 shadow-xs space-y-5">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
            ...
            <div>
                ...
                <input type="text" id="name_ar" name="name_ar" value="{{ old('name_ar', $role->name_ar) }}" required
            ...
            <div>
                ...
                <input type="text" id="name_en" name="name_en" value="{{ old('name_en', $role->name_en) }}" required
                    class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 text-sm text-slate-800 focus:bg-white focus:outline-none focus:border-wood-500 focus:ring-2 focus:ring-wood-500/20 transition">
            </div>
            ...
            <div>
                ...
                <textarea id="description_ar" name="description_ar" rows="2"
                    class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 text-sm text-slate-800 focus:bg-white focus:outline-none focus:border-wood-500 focus:ring-2 focus:ring-wood-500/20 transition">{{ old('description_ar', $role->description_ar) }}</textarea>
            </div>
            ...
            <div>
                ...
                <textarea id="description_en" name="description_en" rows="2"
                    class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 text-sm text-slate-800 focus:bg-white focus:outline-none focus:border-wood-500 focus:ring-2 focus:ring-wood-500/20 transition">{{ old('description_en', $role->description_en) }}</textarea>
            </div>
        </div>
    </div>
