...
@section('page_title', 'إنشاء صفحة مخصصة جديدة')
@section('page_subtitle', 'إدخال محتوى الصفحة وتحديد موضع ظهورها في الموقع مع محرر النصوص الغني')
...
@section('content')
<form method="POST" action="{{ route('admin.pages.store') }}" class="space-y-6" id="pageForm">
    @csrf
    ...
    <div class="bg-white rounded-2xl p-6 sm:p-8 border border-slate-200/80 shadow-xs space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
            <div>
                ...
                <input type="text" id="title_ar" name="title_ar" value="{{ old('title_ar') }}" required
                    ...
                    placeholder="مثال: سياسة الضمان والجودة">
            </div>
            ...
            <div>
                ...
                <input type="text" id="title_en" name="title_en" value="{{ old('title_en') }}" required
                    class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 text-sm text-slate-800 focus:bg-white focus:outline-none focus:border-wood-500 focus:ring-2 focus:ring-wood-500/20 transition"
                    placeholder="e.g. Warranty & Quality Policy">
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
            <div>
                ...
                <select name="placement" id="placement" required class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 text-xs text-slate-800 focus:bg-white focus:outline-none focus:border-wood-500 transition">
                    <option value="both" {{ old('placement') == 'both' ? 'selected' : '' }}>الهيدر والفوتر معاً (Header & Footer)</option>
                    <option value="navbar" {{ old('placement') == 'navbar' ? 'selected' : '' }}>في القائمة العلوية فقط (Navbar Only)</option>
                    <option value="footer" {{ old('placement') == 'footer' ? 'selected' : '' }}>في أسفل الموقع فقط (Footer Only)</option>
                    <option value="none" {{ old('placement') == 'none' ? 'selected' : '' }}>صفحة مباشرة بدون رابط (Direct URL Only)</option>
                </select>
            </div>
        ...
        <div class="pt-2 border-t border-slate-100">
            <label class="flex items-center gap-2 cursor-pointer select-none">
                ...
                <span class="text-xs font-semibold text-slate-700">تفعيل ونشر الصفحة فوراً</span>
            </label>
        </div>
    </div>
